 ...
 */
public class OldWCObjectStoreCoordinatorPool
{
    //~ Constructors ..........................................................
     ...
     */
    private OldWCObjectStoreCoordinatorPool()
    {
        _maxOS = ERXProperties.intForKey(
            OldWCObjectStoreCoordinatorPool.class.getName()
            + ".maxCoordinators");
        if (_maxOS == 0)
        {
    ...
    public static OldWCObjectStoreCoordinatorPool _pool()
    {
        return _pool;
    }
    ...
    public static void initializeIfNecessary()
    {
        ...
        {
            OldWCObjectStoreCoordinatorPool.initialize();
        }
    }
    ...
    public static void initialize()
    {
        if (_pool == null)
        {
            ...
            _pool = new OldWCObjectStoreCoordinatorPool();
        ...
        if (sessionID != null)
        {
            ...
            else
            {
                ...
                EOObjectStore threadOs = (EOObjectStore) ERXThreadStorage
                    .valueForKey(OldWCObjectStoreCoordinatorPool.THREAD_OSC_KEY);
                ...
                ERXThreadStorage.takeValueForKey(
                    os, OldWCObjectStoreCoordinatorPool.THREAD_OSC_KEY);
            }
        }
        else
        {
    ...
    protected EOObjectStore currentThreadObjectStore()
    {
        EOObjectStore os = (EOObjectStore) ERXThreadStorage.valueForKey(
            OldWCObjectStoreCoordinatorPool.THREAD_OSC_KEY);
        if (os == null)
        {
            WCObjectStoreCoordinator.ThreadLockedObjectStore threadLocked =
                WCObjectStoreCoordinator.threadHasLockedStore();
    ...
    static Logger log = Logger.getLogger(OldWCObjectStoreCoordinatorPool.class);
    private static final String THREAD_OSC_KEY =
        OldWCObjectStoreCoordinatorPool.class.getName() + ".threadOSC";
    ...
    protected static OldWCObjectStoreCoordinatorPool _pool;
