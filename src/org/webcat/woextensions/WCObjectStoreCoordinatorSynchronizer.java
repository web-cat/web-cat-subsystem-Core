...
import er.extensions.eof.ERXObjectStoreCoordinatorSynchronizer.Change;
 ...
 * manually.
 */
public class WCObjectStoreCoordinatorSynchronizer
{
	public static final Logger log = Logger.getLogger(
	    WCObjectStoreCoordinatorSynchronizer.class);

	public static final String SYNCHRONIZER_KEY = "_synchronizer";

	private static WCObjectStoreCoordinatorSynchronizer _synchronizer;
	private static ThreadLocal<Boolean> _processingRemoteNotifications =
	...
	public static void initialize()
	{
		...
		{
			_synchronizer = new WCObjectStoreCoordinatorSynchronizer();
		}
	}

	public static WCObjectStoreCoordinatorSynchronizer synchronizer()
	{
		if (_synchronizer == null)
		{
			initialize();
	...
	private static NSMutableDictionary<EODatabase, NSMutableArray<CacheChange>> _cacheChanges =
	    new NSMutableDictionary<EODatabase, NSMutableArray<CacheChange>>();
	private NSMutableArray<EOObjectStoreCoordinator> _coordinators;
	...
	private NSMutableDictionary<EOObjectStoreCoordinator, SynchronizerSettings>
	    _settings;

	private WCObjectStoreCoordinatorSynchronizer()
	{
		_coordinators = new NSMutableArray<EOObjectStoreCoordinator>();
		...
		NSNotificationCenter.defaultCenter().addObserver(this,
		    new NSSelector<Void>("objectStoreWasAdded",
		        ERXConstant.NotificationClassArray),
		    EOObjectStoreCoordinator.CooperatingObjectStoreWasAddedNotification,
		    null);
		NSNotificationCenter.defaultCenter().addObserver(this,
		    new NSSelector<Void>("objectStoreWasRemoved",
		        ERXConstant.NotificationClassArray),
		    EOObjectStoreCoordinator.CooperatingObjectStoreWasRemovedNotification,
		    null);
		NSNotificationCenter.defaultCenter().addObserver(this,
		    new NSSelector<Void>("startRemoteSynchronizer",
		        ERXConstant.NotificationClassArray),
		    WOApplication.ApplicationDidFinishLaunchingNotification,
		    null);
//		NSNotificationCenter.defaultCenter().addObserver(this,
//		    new NSSelector<Void>("stopRemoteSynchronizer",
//		        ERXConstant.NotificationClassArray),
//		    ERXApplication.ApplicationWillTerminateNotification,
//		    null);
	}
	...
	public void setSettingsForCoordinator(
	    SynchronizerSettings settings, EOObjectStoreCoordinator coordinator)
	{
		if (settings == null)
		{
	...
	public SynchronizerSettings settingsForCoordinator(
	...
	{
		boolean remoteSynchronizerEnabled =
		    ERXRemoteSynchronizer.remoteSynchronizerEnabled();
		if (remoteSynchronizerEnabled)
		{
			try
			{
				_remoteSynchronizer =
				    ERXRemoteSynchronizer.newRemoteSynchronizer(_queue);
			...
			catch (Throwable e)
			{
				throw new RuntimeException(
				    "Failed to configure remote synchronization.", e);
			}
		}
	}
	...
	public void addObjectStore(EOObjectStoreCoordinator osc)
	{
		synchronized (_coordinators)
		{
			if (!_coordinators.containsObject(osc))
			{
				...
				NSSelector<Void> sel = new NSSelector<Void>("publishChange",
				    new Class[] { NSNotification.class });
				NSNotificationCenter.defaultCenter().addObserver(this, sel,
				    EOObjectStore.ObjectsChangedInStoreNotification, osc);
				NSSelector<Void> snapshotCacheChanged = new NSSelector<Void>(
				    "snapshotCacheChanged",
				    new Class[] { NSNotification.class });
				NSNotificationCenter.defaultCenter().addObserver(this,
				    snapshotCacheChanged,
				    ERXDatabase.SnapshotCacheChanged,
				    null);
			}
		}
	}
	...
	protected static void setProcessingRemoteNotifications(
	    boolean processingRemoteNotifications)
	{
		_processingRemoteNotifications.set(
		    Boolean.valueOf(processingRemoteNotifications));
	}

	protected static boolean isProcessingRemoteNotifications()
	{
		Boolean processingRemoteNotifications =
		    _processingRemoteNotifications.get();
		return processingRemoteNotifications != null
		    && processingRemoteNotifications.booleanValue();
	}

	public void publishChange(NSNotification n)
	{
		boolean processingMulticastNotifications =
		    isProcessingRemoteNotifications();
		if (_coordinators.count() > 1
		    || (_remoteSynchronizer != null
		        && !processingMulticastNotifications))
		{
			EOObjectStoreCoordinator osc =
			    (EOObjectStoreCoordinator) n.object();
			NSDictionary<String, Object> userInfo = n.userInfo();
			if (userInfo == null
			    || userInfo.valueForKey(SYNCHRONIZER_KEY) == null)
			{
				LocalChange changes = new LocalChange(
				    osc, userInfo, processingMulticastNotifications);
	...
	public void snapshotCacheChanged(NSNotification n)
	{
		...
		if (_remoteSynchronizer != null && !processingMulticastNotifications)
		{
			...
			ERXDatabase.CacheChange cacheChange = (ERXDatabase.CacheChange)
			    n.userInfo().objectForKey(ERXDatabase.CacheChangeKey);
			if (cacheChange != null)
			{
				_enqueueCacheChange(database, cacheChange);
			}
		}
	}

	public synchronized static void _enqueueCacheChange(
	    EODatabase database, CacheChange cacheChange)
	{
		// System.out.println("ERXObjectStoreCoordinatorSynchronizer._enqueueCacheChange:
		// " + cacheChange);
		NSMutableArray<CacheChange> cacheChanges =
		    _cacheChanges.objectForKey(database);
		if (cacheChanges == null)
		{
			cacheChanges = new NSMutableArray<CacheChange>();
			_cacheChanges.setObjectForKey(cacheChanges, database);
		}
		cacheChanges.addObject(cacheChange);
	}

	public synchronized static NSArray<CacheChange> dequeueCacheChanges(
	    EODatabase database)
	{
		NSMutableArray<CacheChange> cacheChanges =
		    _cacheChanges.removeObjectForKey(database);
		return cacheChanges;
	}

	private Enumeration<EOObjectStoreCoordinator> coordinators()
	{
		return _coordinators.objectEnumerator();
	}

	private NSArray<EOObjectStoreCoordinator> _coordinators()
	{
	...
	private class ProcessChangesQueue
	    implements Runnable,
	    er.extensions.eof.ERXObjectStoreCoordinatorSynchronizer.IChangeListener
	{
		...
		private abstract class SnapshotProcessor
		{
			...
			public abstract void processSnapshots(
			    EODatabaseContext dbc,
			    EODatabase database,
			    NSDictionary snapshots,
			    SynchronizerSettings settings);
		}
		    ...
		    extends SnapshotProcessor
		{
			...
			public abstract void processGID(
			    EODatabase database,
			    EOGlobalID sourceGID,
			    Object context,
			    SynchronizerSettings settings);

			public abstract void processRelationship(
			    EODatabase database,
			    EOGlobalID sourceGID,
			    EORelationship sourceRelationship,
			    EOGlobalID destGID,
			    ...
			    SynchronizerSettings settings)
			{
				if (settings.broadcastUpdates())
				{
					...
					NSMutableDictionary userInfo = new NSMutableDictionary(
					    gids, EODatabaseContext.UpdatedKey);
					userInfo.setObjectForKey(Boolean.TRUE,
					    WCObjectStoreCoordinatorSynchronizer.SYNCHRONIZER_KEY);
					NSNotificationCenter.defaultCenter().postNotification(
					    EODatabaseContext.ObjectsChangedInStoreNotification,
					    dbc,
					    userInfo);
		    ...
		    extends RelationshipSnapshotProcessor
		{
			    ...
			    SynchronizerSettings settings)
			{
				...
				NSArray inverseRelationshipGIDs =
				    database.snapshotForSourceGlobalID(
				        destGID, inverseRelationshipName);
				if (inverseRelationshipGIDs != null)
				{
					database.recordSnapshotForSourceGlobalID(
					    inverseRelationshipGIDs.arrayByAddingObject(sourceGID),
					    destGID,
			    ...
			    SynchronizerSettings settings)
			{
				...
				if (settings.broadcastInserts()
				    || settings.broadcastRelationships())
				{
					NSMutableDictionary userInfo = new NSMutableDictionary(
					    gids, EODatabaseContext.InsertedKey);
					userInfo.setObjectForKey(Boolean.TRUE,
					    WCObjectStoreCoordinatorSynchronizer.SYNCHRONIZER_KEY);
					NSNotificationCenter.defaultCenter().postNotification(
					    EODatabaseContext.ObjectsChangedInStoreNotification,
		...
		private abstract class CacheChangeProcessor
		{
			...
			public abstract void processCacheChange(
			    EODatabaseContext dbc,
			    EODatabase database,
		    ...
		    extends CacheChangeProcessor
		{
			...
			@Override
			public void processCacheChange(
			    EODatabaseContext dbc,
			    EODatabase database,
			    CacheChange cacheChange)
			{
				ERXDatabase.SnapshotDeleted deleteChange =
				    (ERXDatabase.SnapshotDeleted) cacheChange;
				EOGlobalID gid = deleteChange.gid();
				WCObjectStoreCoordinatorSynchronizer
				    .setProcessingRemoteNotifications(true);
				try
				{
				...
				{
					WCObjectStoreCoordinatorSynchronizer
					    .setProcessingRemoteNotifications(false);
				}
				if (log.isDebugEnabled())
				{
		    ...
		    extends CacheChangeProcessor
		{
			...
			public void processCacheChange(
			    ...
			    CacheChange cacheChange)
			{
				ERXDatabase.SnapshotUpdated updateChange =
				    (ERXDatabase.SnapshotUpdated) cacheChange;
				...
				if (snapshot != null)
				{
					...
					try
					{
						EOEditingContext editingContext =
						    WCEC.newEditingContext();
						editingContext.lock();
						try
						{
							ERXEOGlobalIDUtilities.fetchObjectsWithGlobalIDs(
							    editingContext, new NSArray(gid), true);
						}
						finally
						{
						...
						// local notify of remote updates. See
						// https://github.com/wocommunity/wonder/issues/866
						NSMutableDictionary userInfo = new NSMutableDictionary(
						    new NSArray(gid), EODatabaseContext.UpdatedKey);
						userInfo.setObjectForKey(Boolean.TRUE,
						    WCObjectStoreCoordinatorSynchronizer.SYNCHRONIZER_KEY);
						NSNotificationCenter.defaultCenter().postNotification(
						    EODatabaseContext.ObjectsChangedInStoreNotification,
		    ...
		    extends CacheChangeProcessor
		{
			...
			public void processCacheChange(
			    ...
			    CacheChange cacheChange)
			{
				ERXDatabase.SnapshotInserted insertChange =
				    (ERXDatabase.SnapshotInserted) cacheChange;
		    ...
		    extends CacheChangeProcessor
		{
			...
			public void processCacheChange(
			    ...
			    CacheChange cacheChange)
			{
				ERXDatabase.ToManySnapshotUpdated toManyUpdateChange =
				    (ERXDatabase.ToManySnapshotUpdated) cacheChange;
				WCObjectStoreCoordinatorSynchronizer
				    .setProcessingRemoteNotifications(true);
				try
				{
					...
					database.recordSnapshotForSourceGlobalID(
					    null, sourceGID, toManyUpdateChange.name());
		...
		private SnapshotProcessor _deleteProcessor =
		    new DeleteSnapshotProcessor();
		private SnapshotProcessor _insertProcessor =
		    new InsertSnapshotProcessor();
		private SnapshotProcessor _updateProcessor =
		    new UpdateSnapshotProcessor();
		private SnapshotProcessor _invalidateProcessor =
		    new InvalidateSnapshotProcessor();
		private CacheChangeProcessor _insertCacheChangeProcessor =
		    new InsertCacheChangeProcessor();
		private CacheChangeProcessor _updateCacheChangeProcessor =
		    new UpdateCacheChangeProcessor();
		private CacheChangeProcessor _deleteCacheChangeProcessor =
		    new DeleteCacheChangeProcessor();
		private CacheChangeProcessor _toManyUpdateCacheChangeProcessor =
		    new ToManyUpdateCacheChangeProcessor();
		...
		protected void _process(
		    EOObjectStoreCoordinator sender,
		    EOObjectStoreCoordinator osc,
		    ...
		    String userInfoKey)
		{
			...
			EOModelGroup modelGroup =
			    EOModelGroup.modelGroupForObjectStoreCoordinator(sender);
			for (Enumeration entityNames =
			    changesByEntity.allKeys().objectEnumerator();
			    entityNames.hasMoreElements();)
			{
				...
				NSArray snapshots =
				    (NSArray) changesByEntity.objectForKey(entityName);
				EODatabaseContext dbc =
				    (EODatabaseContext) dbcs.objectForKey(key);
				if (dbc == null)
				{
					dbc = ERXEOAccessUtilities.databaseContextForEntityNamed(
					    osc, entityName);
					dbcs.setObjectForKey(dbc, key);
				}
				EODatabase database = dbc.database();
				NSMutableDictionary snapshotsByGlobalID =
				    new NSMutableDictionary();
				for (Enumeration snapshotsEnumerator =
				    snapshots.objectEnumerator();
				    snapshotsEnumerator.hasMoreElements();)
				{
					NSDictionary snapshot =
					    (NSDictionary) snapshotsEnumerator.nextElement();
				...
				if (snapshotsByGlobalID.count() > 0)
				{
					...
					try
					{
						processor.processSnapshots(
						    dbc, database, snapshotsByGlobalID, settings);
					}
					finally
					{
						dbc.unlock();
		    ...
		    String userInfoKey)
		{
			...
			for (Enumeration oscs = _synchronizer.coordinators();
			    oscs.hasMoreElements();)
			{
				EOObjectStoreCoordinator osc =
				    (EOObjectStoreCoordinator) oscs.nextElement();
				if (osc != sender)
				{
					_process(sender, osc, dbcs, processor,
					    changesByEntity, userInfoKey);
				}
			}
		}
		...
		protected void processRemoteChange(RemoteChange remoteChange)
		{
			    ...
			    oscs.hasMoreElements();)
			{
				EOObjectStoreCoordinator osc =
				    (EOObjectStoreCoordinator) oscs.nextElement();
				Enumeration cacheChangeEnum =
				    remoteChange.remoteCacheChanges().objectEnumerator();
				while (cacheChangeEnum.hasMoreElements())
				{
					ERXDatabase.CacheChange cacheChange =
					    (ERXDatabase.CacheChange) cacheChangeEnum.nextElement();
					EOKeyGlobalID gid = (EOKeyGlobalID) cacheChange.gid();
					EODatabaseContext dbc = ERXEOAccessUtilities
					    .databaseContextForEntityNamed(osc, gid.entityName());
					EODatabase database = dbc.database();
					dbc.lock();
					try
					{
		...
		protected void publishRemoteChanges(
		    int transactionID, LocalChange localChange)
		{
			if (_remoteSynchronizer != null
			    && !localChange.causedByRemoteUpdate())
			{
				try
				{
					...
					{
						_remoteSynchronizer.writeCacheChanges(
						    transactionID, cacheChanges);
					}
				}
				catch (Throwable e)
				{
					log.error("Failed to send remote changes: "
					    + localChange + ".", e);
				}
			}
		}
				...
				while (_running)
				{
					...
					if (changes != null)
					{
						if (changes instanceof LocalChange)
						{
							...
							EOObjectStoreCoordinator sender =
							    localChange.coordinator();
							process(sender,
							    _deleteProcessor,
							    localChange.deleted(),
							    EODatabaseContext.DeletedKey);
							process(sender,
							    _insertProcessor,
							    localChange.inserted(),
							    EODatabaseContext.InsertedKey);
							process(sender,
							    _updateProcessor,
							    localChange.updated(),
							    EODatabaseContext.UpdatedKey);
							process(sender,
							    _invalidateProcessor,
							    localChange.invalidated(),
							    EODatabaseContext.InvalidatedKey);
		...
		public void stop()
		{
			...
			{
				log.warn("Attempted to stop the " + getClass().getSimpleName()
				    + " when it wasn't already running");
			}
		}
	}
	    ...
	    extends Change
	{
		...
		public RemoteChange(
		    String identifier, int transactionID, int transactionSize)
		{
	...
	public static class LocalChange extends Change
	{
		    ...
		    boolean causedByRemoteUpdate)
		{
			_coordinator = osc;
			_deletedGIDs =
			    (NSArray) userInfo.objectForKey(EOObjectStore.DeletedKey);
			_updatedGIDs =
			    (NSArray) userInfo.objectForKey(EOObjectStore.UpdatedKey);
			_insertedGIDs =
			    (NSArray) userInfo.objectForKey(EOObjectStore.InsertedKey);
			_invalidatedGIDs =
			    (NSArray) userInfo.objectForKey(EOObjectStore.InvalidatedKey);
		 ...
		 * @param osc
		 */
		protected NSDictionary snapshotsGroupedByEntity(
		    NSArray objects, EOObjectStoreCoordinator osc)
		{
			...
			for (Enumeration gids = objects.objectEnumerator();
			    gids.hasMoreElements();)
			{
				...
				if (gid instanceof EOKeyGlobalID)
				{
					...
					if (dbc == null)
					{
						dbc = ERXEOAccessUtilities
						    .databaseContextForEntityNamed(osc, entityName);
						dbcs.setObjectForKey(dbc, key);
					}
					NSMutableArray snapshotsForEntity =
					    (NSMutableArray) result.objectForKey(entityName);
	...
	public static class SynchronizerSettings
	{
		...
		public SynchronizerSettings(
		    boolean broadcastInserts,
		    boolean broadcastUpdates,
		    boolean broadcastDeletes,
		    boolean broadcastRelationships)
		{
